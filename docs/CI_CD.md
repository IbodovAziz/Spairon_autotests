...
- `Мониторинг production` — критический независимый production-safe набор с
...
автоматическую доставку тестовых отчётов и ежечасную непрерывную проверку
production.
...
4. Запускает 11 публичных smoke-тестов авторизации и регистрации.
...
- каждый час в начале часа по расписанию `0 * * * *`;
- вручную через GitHub Actions.

Workflow запускает активный набор из 16 независимых тестов одним worker:

- 11 публичных проверок авторизации и регистрации;
- 2 проверки входа и выхода;
- 3 проверки статуса и доступности HeadHunter.

Вакансии и анализ временно исключены из CI. Общий setup авторизации не
используется: каждый авторизованный тест самостоятельно входит в новом browser
context. Поэтому падение одного кейса не переводит остальные в `Skipped`.
Параллельные production-запуски запрещены.

Push и pull request не запускают production workflow. Они запускают только
публичный CI. Для внеплановой полной проверки используйте `Run workflow`.

Расписание GitHub Actions использует UTC, но выражение запускает тесты каждый
час независимо от часового пояса. GitHub может начать scheduled job с небольшой
задержкой при высокой нагрузке.
...
- непосредственную причину падения: HTTP-статус, сообщение API и
  `Expected/Received`;
...
данные тестовой учётной записи и статус интеграции. Поэтому production workflow:
