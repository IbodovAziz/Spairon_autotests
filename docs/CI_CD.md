...
- `Public Smoke CI` — быстрая проверка публичных экранов и API;
- `Production Smoke Monitoring` — полный production-safe набор с
...
- вручную через `Actions → Public Smoke CI → Run workflow`.
...
- после успешного `Public Smoke CI` для push в `main`;
...
- статус;
- total, passed, failed, flaky и skipped;
- длительность;
- ссылку на workflow;
- ссылку на artifact отчёта.
...
- выбрать check `Public Playwright smoke`;
